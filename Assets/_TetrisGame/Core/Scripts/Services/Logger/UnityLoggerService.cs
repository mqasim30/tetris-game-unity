...
using TetrisGame.Core.Scripts.Extensions;
using UnityEngine;

namespace TetrisGame.Core.Scripts.Services.Logger

{
    public class UnityLoggerService : LoggerBase
    {
        ...
        public override void LogTopic(string message, LogTopicType debugLogTopic = LogTopicType.Temp,
            string callerFilePath = "", string callerMemberName = "")
        {
            Debug.Log(debugLogTopic + DebugTopicSuffix + GetTimeStamp() +
                      StampFormat.Format(GetCallerName(callerFilePath) + Dot + callerMemberName) + " " + message);
        }
